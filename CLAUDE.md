...
**Spellcheck & Autocorrect**: `SpellChecker` in `chat/spellcheck/checker.py` wraps hunspell (Linux, fast C extension) or pyspellchecker (cross-platform, pure Python fallback) with chat-aware skip rules (emotes, URLs, mentions, all-caps). Red wavy underlines drawn in `ChatInput.paintEvent()`. `SpellCompleter` shows click-to-correct popup. Autocorrect (`get_confident_correction`) auto-replaces obvious typos when the user moves past a misspelled word (space + next letter). The decision itself lives in the pure `choose_confident_correction(word, suggestions)` so it unit-tests without a backend. It fires only when: an apostrophe expansion matches (dont→don't), or there is exactly 1 suggestion, or the top suggestion is within Damerau-Levenshtein distance 1 — **and** the word is not capitalized (names: `Kaydop` was being rewritten to `Gaydo`) **and** the replacement is a single word (`Arejay` was becoming `Are jay`). Apostrophe expansions are exempt from the capitalization rule so `Dont`→`Don't` still works. Autocorrect additionally skips any word the caret is currently inside (`caret_inside`), and applies replacements with `setSelection`+`insert` rather than `setText` so Ctrl+Z can revert them. Bundled adult word list (`data/adult.txt`) prevents profanity from being flagged. Custom dictionary additions sync to backend runtime via callback. Corrected words show a green underline for 3 seconds. Both features togglable in Preferences > Chat and the gear menu.
...
| Taskbar icon ignores the runtime theme | Setting `setDesktopFileName()` makes the desktop environment resolve the taskbar/launcher icon from the `.desktop` file's `Icon=` key, which points at the static `data/*.svg`. The theme-derived `create_app_icon()` only reaches the tray and the window icon. A static file cannot follow the theme, so `data/app.livestreamlist.LivestreamListQt.svg` is deliberately neutral charcoal rather than a saturated brand colour. Regenerate `data/icon.ico` from the SVG whenever it changes, or Windows drifts out of sync. |
| Autocorrect fights the user editing a word | `_run_check` decided purely on "is there a space+letter after this word", ignoring the caret. Moving the cursor back into a word and deleting a character made the shortened word a *new* misspelling, which was rewritten mid-edit with the caret flung past it. Guard with `caret_inside(start, end, caret)`. The Tauri sibling documents the same bug in `src/utils/autocorrect.js` — it was ported from this code and fixed there first. |
| `QLineEdit.setText()` clears the undo stack | Applying autocorrect with `setText()` left `isUndoAvailable() == False`, so a wrong correction could not be taken back. Use `setSelection(start, n)` + `insert(replacement)` — it goes through the undo stack and Ctrl+Z reverts. Applies to any programmatic edit of user-entered text. |
